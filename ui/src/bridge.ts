...
export function probe(): unknown {
  ...
  return {
    ...
    domSatellites: countAll(".sat"),
    domCallEdges: countAll(".react-flow__edge.callEdge"),
    domImplEdges: countAll(".react-flow__edge.implements"),
    expandedNode: document.querySelector('.step-node[data-papel="expandido"]')?.getAttribute("data-order") ?? null,
    dimmedNodes: document.querySelectorAll('.step-node[data-papel="apagado"]').length,
    highlightedCalls: document.querySelectorAll('.step-node[data-papel="chamado"]').length,
